...
from output_parsers import summary_parser, net_meet_parser, topics_of_interest_parser
...
def get_net_meet_chain() -> LLMChain:
    net_meet_template = """
         given the information about a person from linkedin {information}, and twitter posts {twitter_posts} I want you to create:
    ...
    net_meet_prompt_template = PromptTemplate(
        input_variables=["information", "twitter_posts"],
        template=net_meet_template,
        partial_variables={
            "format_instructions": net_meet_parser.get_format_instructions()
        },
    )

    return LLMChain(llm=llm_creative, prompt=net_meet_prompt_template)
